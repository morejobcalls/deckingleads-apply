...
- **2026-09-15 (later, Spencer):** `/yt/` repositioned **contractor-general** (eyebrow "For Established Contractors", "10+ Exclusive Sales Appointments", "1 contractor per trade, per market", "more jobs", contractor-wide trust/wins headings) because the YouTube TOF content addresses the whole contractor TAM and deck-only copy was repelling other trades. **Organic guarantee ≠ 100-in-100:** `/yt/` and its twins now carry "30 Minimum On-Site Appointments – Guaranteed" + "Or Your Money Back" (fee-back scoped to the management fee); no $10,000 / day-100 / 20-jobs copy anywhere on the organic pages. Client proof (names, logos, result captions) left verbatim. Both passes live in `build_yt_lp.py` (`GENERAL_*`, `GUAR_*`) and assert on every re-run.

---

## 2026-09-18 — One organic link per channel: `/yt/` · `/ig/` · `/fb/` (SHIPPED)

- **Why:** Chung flagged (Basecamp, 9/17) that the Facebook post's link was `/yt/`. `/post-content` put the YouTube link on every platform, and the fb-page-legit routine sent Facebook page posts to the PAID root page (which fires paid `Lead` events).
- **Pages:** `/ig/` (the stale 2026-08-01 v1 clone; its uncommitted edits saved as `capi-worker/workflow-backups/2026-09-18/ig_uncommitted_BEFORE_rebuild.patch`) was REBUILT, and `/fb/` is NEW. Both are generated with `/yt/` by `capi-worker/workflow-backups/2026-09-18/build_organic_lps.py` (supersedes `build_yt_lp.py`) from committed root v9, so all three share one design/survey/organic copy (contractor-general, 30-appointment money-back guarantee). `/yt/` also picked up the 9/17 root changes (H1, trust strip, proof-reel section).
  - `/ig/`: `utm_source=instagram / organic / ig-bio`, `?p=` → utm_content, `IG_Lead` / `IG_BookedCall`. Link lives in the IG bio.
  - `/fb/`: `utm_source=facebook / organic / fb-page`, `?p=` → utm_content, `FB_Lead` / `FB_BookedCall`.
